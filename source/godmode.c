...
u32 FileHandlerMenu(char* current_path, u32* cursor, u32* scroll, DirStruct* current_dir, DirStruct* clipboard) {
    ...
    if (user_select == mount) { // -> mount file as image
        ...
        if (ShowUnlockSequence(3, "%s (%dkB)\nBoot FIRM?\n", pathstr, payload_size / 1024)) {
            FileGetData(curr_entry->path, TEMP_BUFFER, payload_size, 0);
            if (ValidateFirm(TEMP_BUFFER, payload_size)) {
                BootFirm(TEMP_BUFFER, curr_entry->path);
                while(1);
            } else {
                ShowPrompt(false, "FIRM validation failed.\n");
            }
        }
        return 0;
